...
users = session.query(User).filter_by(age = 25).one_or_none() # you can also use one_or_none() method to access the single user if you know that there is only single user in the database(Table). and if you have many this will give you an error message
