...
users = session.query(User).order_by(User.age, User.name).all() # Take a note the first coulumn is age and the second column is name it will start the sort the starting age which is 20 and then in the group of 20 ages users it will sort the name in ascending order  
